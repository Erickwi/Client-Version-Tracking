...
export function VersionTimeline({ history }: VersionTimelineProps) {
  // `latestVersion` is not used here — keep router/component logic simple

  if (history.length === 0) {
    return <p className="text-sm text-muted-foreground py-8 text-center">Sin historial de versiones registrado.</p>;
  }
        ...
        return (
          <li key={entry.id} className="ml-8">
            ...
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              {isCurrent && (
                <span className="text-[11px] font-medium text-primary tracking-wider uppercase">— Actual</span>
              )}
            </div>
            ...
            {entry.notes && <p className="mt-2 text-sm text-muted-foreground/80 leading-relaxed">{entry.notes}</p>}
          </li>
        );
      })}
    </ol>
